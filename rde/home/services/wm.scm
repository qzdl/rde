...
(define-configuration home-i3-configuration
  (package
    ...
    "i3 package to use.")
  (config
   ;;; TODO: make the default value thunked and use a i3 package
   ;;; from package field. Requires patching define-configuration
   ;; `((include ,(file-append
   ;; 		 (home-i3-configuration-package this-home-i3-configuration)
   ;; 		 "/etc/i3/config")))
   (i3-config
...
There is no special syntax for CRITERIA (See @command{man 5 i3}),
because i3's doc states it's a string, so use a string for that.
...
(define home-i3-service-type
  (service-type (name 'home-i3)
		...
		(extend home-i3-extensions)
                ...
                (description "\
Install and configure i3, a tiling window manager on X11")))
